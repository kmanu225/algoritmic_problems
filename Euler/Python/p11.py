    ...
    for k in range(0, lim - 3):
        ...
        while i != lim - 3:
            if j < lim - 3:
                ...
                pmax = max(
                    pmax, A[i][j] * A[i + 1][j + 1] * A[i + 2][j + 2] * A[i + 3][j + 3]
                )
...
def ortho(M):
    oM = np.arange(len(M) * len(M)).reshape(len(M), len(M))

    for i in range(len(M)):
        for j in range(len(M)):
            oM[i][j] = M[i][len(M) - 1 - j]
    return oM
...
print(
    findmaxi2(
        ortho(M).transpose(),
        findmaxi2(ortho(M), findmaxi1(M.transpose(), findmaxi1(M))),
    )
)
print(time() - start)
